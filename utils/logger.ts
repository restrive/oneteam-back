...
export function cLog(src: string, info: string, errorLevel: string = "debug", cLogTrue: boolean = true) {
    ...
    switch (writeFileLevel) {
        case "debug": writeToFile(src, info, errorLevel); if (writeToDb === 'true') { writeToDB(src, info, errorLevel) }
            break;
        case "warning": if (errorLevel !== "debug") { writeToFile(src, info, errorLevel); if (writeToDb === 'true') { writeToDB(src, info, errorLevel) } }
            break;
        case "error": if (errorLevel !== "debug" && errorLevel !== "warning") { writeToFile(src, info, errorLevel); if (writeToDb === 'true') { writeToDB(src, info, errorLevel) } }
            break;
        case "critical": if (errorLevel === "critical") { writeToFile(src, info, errorLevel); if (writeToDb === 'true') { writeToDB(src, info, errorLevel) } }
            break;
    }
    switch (sendEmail) {
...
async function writeToDB(src: string, info: string, errorLevel: string = "debug") {
    const results = await db.asyncPool(`INSERT INTO logs (src, info, error_level) VALUES ('${src}', '${info}', '${errorLevel}')`, []);
    console.log(results)
}
